...
png(args[2], width=500, height=471)

ggplot(x, aes(x=nth.genome, y=base.pairs/1e9)) + geom_point(alpha=I(1/10)) + stat_function(fun=function(x) (fit$par[1] * x^fit$par[2] + fit$par[3]) * m) + scale_y_continuous("observed pangenome size (Gbp)") + scale_x_continuous(paste("Nth genome (", max(x$permutation)+1 ," permut.), gamma=", round(fit$par[2], digits=3), sep = "")) + theme(text = element_text(size=21))
